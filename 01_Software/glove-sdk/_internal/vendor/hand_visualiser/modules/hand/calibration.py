...
from _handmodel import AxisLayerFK
from scipy.spatial.transform import Rotation as R
from _handmodel import HandLayer, HandOutput
...
from modules.utils import frame_to_world
...
class WristPoseCalibrator:
    ...
    def encode(self):
        return {
            ...
            "distal_pos_hand": self._distal_pos_hand.tolist(),
    ...
    def decode(cls, data: dict):
        return cls(
            ...
            distal_pos_hand=np.array(data['distal_pos_hand']),
    ...
    def get_wrist_transformation_matrix(
        ...
        hand_transformation_matrix = rigidbody_transmat @ self._t_mat_inv

        return R.from_matrix(hand_transformation_matrix[:3, :3]), hand_transformation_matrix[:3, 3]
...
class Raw2HandCalibrator:
    ...
    def __init__(self,
        ...
        self.side = side
        self._axislayer = AxisLayerFK(hand_assets_root="assets/hand_model", side=self.side.value)
    ...
    def _get_rest_pose_coord_diff_matrix(self):
        hand_layer = HandLayer(
            rot_mode="axisang",
            use_pca=False,
            side=self.side.value,
            center_idx=None,
            flat_hand_mean=True,
            hand_assets_root="assets/hand_model",
        )
        bs = 1
        random_shape = torch.rand(bs, 10)
        hand_pose = torch.zeros(bs, 48)
        hand_results: HandOutput = hand_layer(hand_pose, random_shape)
        T_g_p = hand_results.transforms_abs
    ...
    def _get_global_installation_error(self, calibration_quat: CalibrationQuatSet) -> np.ndarray:
        ...
        if self.side == MoCapHandIDEnum.LEFT:
            z_axis *= -1
    ...
    def get_hand_param(self, rotations: R, ana_repr: bool = False, map_to_world: bool = False) -> torch.Tensor:
        assert len(rotations) == 16

        rotvec_hand = np.zeros((16, 3))
        rotvec_fk = np.zeros_like(rotvec_hand)
        ...
        rotvec_hand[0] = (global_inst_error[0] * delta_imu[0] * local_inst_error[0].inv()).as_rotvec()
        rotvec_fk[0] = R.from_rotvec(rotvec_hand[0]).as_rotvec()
        ...
        for idx in self._fk_seq:
            ...
            rotvec_hand[idx] = (cur_rot.inv() * global_inst_error[idx] * delta_imu[idx] * local_inst_error[idx].inv()).as_rotvec()
            rotvec_fk[idx] = (cur_rot * R.from_rotvec(rotvec_hand[idx])).as_rotvec()

        if ana_repr:
            return torch.from_numpy(rotvec_hand)

        ana_euler = R.from_rotvec(rotvec_hand).as_euler('XYZ').reshape(1, 16, 3)
        rot_dest_input = self._axislayer.compose(torch.from_numpy(ana_euler).float())[0]

        if map_to_world:
            rot_dest_input[0] = torch.from_numpy(frame_to_world(R.from_rotvec(rotvec_hand[0])).as_rotvec())
        else:
            rot_dest_input[0] = torch.from_numpy(rotvec_hand[0])

        return rot_dest_input

    def __call__(self, rotation: R, ana_repr: bool = False, map_to_world: bool = False) -> torch.Tensor:
        return self.get_hand_param(rotation, ana_repr, map_to_world)
...
class GPRInstallationCalibrator:
    ...
    def get_hand_param(self, hand_params: torch.Tensor) -> torch.Tensor:
        assert hand_params.shape == (16, 3), "The shape of hand_params must be (16, 3)"
        hand_params_np = hand_params.cpu().numpy()
        res = hand_params_np.copy()
        for idx, gpr in enumerate(self.gprs):
            res[idx] = gpr.predict(hand_params_np[idx, :].reshape(1, -1)).reshape(-1, 3)

        return torch.from_numpy(res).to(hand_params.device)
...
def get_optimized_shape_params(
    ...
    with tqdm(range(iter_num)) as pbar:
        for _ in range(iter_num):
            ...
            hand_results: HandOutput = hand_layer(rot_params.unsqueeze(0), shape_params.unsqueeze(0))
            verts = hand_results.verts.squeeze(0)
            hand_key_pts = verts[keypoints_verts_ids.to(compute_device)]
            hand_root = torch.mean(verts[-2: ], dim=0)
            hand_key_pts = torch.concat([hand_key_pts[:-2], hand_root.unsqueeze(0)], dim=0)

            optimizer.zero_grad()
            loss = loss_f(hand_key_pts, keypoints) + regular_term_weight * torch.linalg.norm(shape_params)
            if _ == 0:
